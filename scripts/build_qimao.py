"""
构建七猫榜单 latest_ranks.json：
1. 加载最近两天的 JSON 快照
2. 按分类对比趋势（新上榜/掉榜/排名变化/热度变化）
...
def generate_trend_summary_text(cat_name: str, trend: dict) -> str:
    ...
    if trend["reads_growth"]:
        ...
        parts.append(f"《{g['title']}》热度{g['growth']}")
    if not parts:
...
def build_ai_prompt(cat_name: str, cat: dict, trend: dict) -> str:
    ...
    return f"""你是一位网文行业分析师。请根据以下数据，为七猫免费小说「{cat_name}」生成结构化分析。
...
GENRE_GROUPS = [
    {"name": "男生榜单", "categories": ["男生新书榜", "男生大热榜", "男生完结榜", "男生收藏榜"]},
    {"name": "女生榜单", "categories": ["女生新书榜", "女生大热榜", "女生完结榜", "女生收藏榜"]},
]

MARKET_KEYWORDS = [
    "重生", "穿越", "系统", "赘婿", "战神", "神豪", "鉴宝", "兵王", "逆袭", "打脸",
    "马甲", "团宠", "萌宝", "闪婚", "追妻", "替嫁", "冲喜", "豪门", "总裁",
    "甜宠", "虐文", "先婚后爱", "真假千金", "嫡女", "宅斗", "宫斗",
    "种田", "年代", "七零", "末世", "废土", "异能", "玄幻", "仙侠", "修真", "高武",
    "御兽", "无限流", "悬疑", "灵异", "规则怪谈", "克苏鲁", "星际", "国运", "直播",
    "娱乐圈", "美食", "经商", "基建", "长生",
]
...
def build_rule_market_summary(period_label: str, hot_genres: list,
    ...
    return (
        ...
        f"具体分类以 {top_types} 的新增热度更集中；新书题材上 {top_themes} "
...
def build_market_ai_prompt(payload: dict) -> str:
    ...
    for key, data in payload.get("periods", {}).items():
        genres = "、".join(
            f"{item['name']}(新增热度{format_market_reads(item.get('read_growth_total', 0))}, "
    ...
    return f"""你是一位网文市场编辑，请根据七猫免费小说榜单（男频+女频）的统计结果，为每个周期生成一段全站热点判断。
...
3. 综合赛道和具体分类必须按“新增热度/热度增长”解读，不要写“榜单动能”或“排名动能”。
...
def main():
    parser = argparse.ArgumentParser(description="构建七猫榜单 latest_ranks.json")
    ...
    # 查找 JSON 快照文件
    snapshots = sorted(
        glob.glob(os.path.join(data_dir, "qimao_ranks_*.json"))
    )

    if not snapshots:
        print("未找到任何 JSON 快照文件。请先运行 scrape_qimao_ranks.py。")
        sys.exit(1)
    ...
    if args.date:
        ...
        candidates = [
            os.path.join(data_dir, f"qimao_ranks_{target_date_compact}.json"),
        ]
    ...
    for s in snapshots:
        ...
        # qimao_ranks_YYYYMMDD.json -> YYYY-MM-DD
