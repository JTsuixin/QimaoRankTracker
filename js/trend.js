document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderMarketBoard(rowsWindow) {
        ...
        const fallbackSummary = `${period}里，${topGenres || topTypes} 的阅读增长更强，具体分类以 ${topTypes} 的新增热度更集中；新书题材上 ${topThemes} 更高频，说明读者仍偏好强设定、强情绪钩子和明确爽点。`;
        ...
        els.hotGenres.innerHTML = hotGenres.slice(0, 5).map((item, index) => `
            <div class="hot-type-row hot-type-row-static genre-row">
                ...
                <small>${escapeHtml(item.categoryText)} · 新增热度 ${formatReads(item.readGrowthTotal)} · 增长作品 ${item.readCount}</small>
        ...
        els.hotTypes.innerHTML = hotTypes.slice(0, 6).map((item, index) => `
            <button class="hot-type-row" type="button" data-type="${escapeAttr(item.name)}">
                <span>${index + 1}</span>
                <strong>${escapeHtml(item.name)}</strong>
                <small>新增热度 ${formatReads(item.readGrowthTotal)} · 增长作品 ${item.readCount}</small>
                <em>${formatReads(item.readGrowthTotal)}</em>
            </button>
        `).join('');
    ...
    function extractBookId(url) {
        const match = String(url || '').match(/shuku\/(\d+)/);
