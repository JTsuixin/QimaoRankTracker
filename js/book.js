document.addEventListener('DOMContentLoaded', () => {
    ...
    function snapshotUrl(date) {
        return `data/qimao_ranks_${date.replace(/-/g, '')}.json`;
    }
...
链接：${book.url || '无'}`;
    ...
    function renderReadsChart(records) {
        if (!records.length) {
            return '<p class="muted-line">暂无可用的热度趋势数据。</p>';
        }
    ...
    function bindReadsChart(records) {
        ...
        function showPoint(index) {
            ...
            tooltip.innerHTML = `
                ...
                <div><i></i><span>热度值</span><strong>${escapeHtml(record.readsLabel)}</strong></div>
            `;
